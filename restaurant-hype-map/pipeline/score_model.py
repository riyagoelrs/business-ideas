...
# STRICTLY ZERO-COST CORE MODEL.
# Missing sources are reweighted; they are never treated as zero.
HYPE_WEIGHTS = {
    "tiktok": 0.65,
    "web": 0.25,
    "reservation": 0.10,  # optional free snapshot, if available
}

QUALITY_WEIGHTS = {
    "beli": 0.85,
    "website": 0.15,  # structured aggregateRating published by the restaurant site, if present
}

# Overall data coverage. This is intentionally source-based so users can see
# whether a score is broad or is being driven by one platform.
COVERAGE_WEIGHTS = {
    "tiktok": 55,
    "web": 25,
    "beli": 15,
...
def score_restaurants(rows: List[dict]) -> List[dict]:
    ...
    universes = {
        ...
        "web_7d": [src(r, "web", "mentions_7d") for r in rows],
        "web_domains": [src(r, "web", "domains_30d") for r in rows],
        "web_growth": [_growth(src(r, "web", "mentions_7d"), src(r, "web", "mentions_prev_7d")) for r in rows],
    }
