...
def run() -> None:
    ...
    for i, row in enumerate(rows, start=1):
        ...
        # Carry forward the last good snapshot for every non-Beli source. A failed
        # API call should lower freshness, not erase yesterday's known data.
        if prev:
            carried = {k: v for k, v in (prev.get("sources") or {}).items() if k != "beli"}
            row["sources"].update(carried)
